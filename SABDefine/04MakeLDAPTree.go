...
delete from ldapx_persons where lang=0 and uid not in (select uid from zzdmp_ora_pers where uid is not null) and uid is not null;
...
insert into ldapx_persons (id,uid,lang)
	select subq.id,zzdmp_ora_pers.uid,1 from zzdmp_ora_pers, (select id,uid from ldapx_persons) as subq
	where zzdmp_ora_pers.uid is not null and zzdmp_ora_pers.uid not in (select uid from ldapx_persons where uid is not null and lang=1) and
...
insert into ldap_entries (dn,oc_map_id,parent,keyval,uid,idparent,pass)
		...
		1, ldap_entries.id, ldapx_persons.id, zzdmp_ora_pers.uid, zzdmp_ora_pers.iddep, 3
			from zzdmp_ora_pers, ldapx_persons, ldap_entries
				where ldapx_persons.lang=0 and zzdmp_ora_pers.uid is not null and zzdmp_ora_pers.uid not in
				(select uid from ldap_entries where uid is not null) and
...
update ldap_entries
		...
		from (select format('CN=%s,%s', regexp_replace(zzdmp_ora_pers.trnamefull, '[^A-Za-z0-9\ \_\-]', '', 'g'), ldap_entries.dn) as dn,
			...
			where ldapx_persons.lang=0 and ldapx_persons.uid=zzdmp_ora_pers.uid and ldap_entries.uid=zzdmp_ora_pers.iddep and
				curr.uid=zzdmp_ora_pers.uid and
...
insert into XYZTempTableZYX (phone, pers_id)
        select format('8%s', regexp_split_to_table(regexp_replace(regexp_replace(ora.phoneint, '[^0-9\n]', '', 'g'), '\n', ',' ,'g'), ',')), pers.id
                from zzdmp_ora_pers as ora, ldapx_persons as pers where pers.lang=0 and pers.uid=ora.uid and ora.phoneint similar to '%[0-9]%';
...
insert into XYZTempTableZYX (phone,pers_id)
        select regexp_split_to_table(regexp_replace(regexp_replace(ora.phonecell, '[^+0-9\n]', '', 'g'), '\n', ',' ,'g'), ','), pers.id
                from zzdmp_ora_pers as ora, ldapx_persons as pers where pers.lang=0 and pers.uid=ora.uid and ora.phonecell similar to '%[0-9]%';
...
`)
	...
	PG_QUE_LDAP_ORGS41	=	[]string {`
insert into ldapx_mail (mail, pers_id) select distinct ml.mail, pers.id from zzdmp_domino as ml, zzdmp_ora_pers as ora, ldapx_persons as pers
	where
		(ml.namerus=ora.namefull or ml.namerus=ora.namelf or ml.namerus=ora.namelfi or ml.trnamerus=ora.trnamefull or
		ml.trnamerus=ora.trnamelf or ml.trnamerus=ora.trnamelfi) and ora.uid=pers.uid and pers.lang=0 and ml.mail not in
		(select mail from ldapx_mail where mail is not null);
`}
